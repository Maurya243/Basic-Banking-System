 ...
 if($amnt > $sql1[3])
    {
        ...
        $balance =$sql1[3] -$amnt;
        $sql = "UPDATE customers set Amount=$balance where custID=$from";
        mysqli_query($con,$sql);



        $balance =$sql2[3] +$amnt;
        $sql = "UPDATE customers set Amount=$balance where custID=$to";
        mysqli_query($con,$sql);
...
<body>
        ...
        <select class=" form-control"   name="to" style="margin-bottom:5%;" required>
            ...
            ?>
                <option class="table text-center table-striped " value="<?php echo $rows['custID'];?>" >
            ...
            <button class="btn btn-success btn-sm active" name="submit" type="submit" id="mybtn" style="margin-left:5%">Proceed</button>
            </div>
        </form>
    </div>
 </body>      
